"""
Stage 12: Compute aggregated article page variables for each WikiProject.

For each WikiProject, iterates over all its member article pages and
accumulates monthly edit statistics, then saves one CSV per project.
...
Usage:
  python scripts/03_compute_article_page_variables.py          # demo
...
from collection_utils import (
    ...
    init_aggregated_article_project_dict,
    compute_article_page_variables,
)
...
# These are set once per worker process via the pool initializer
_bots_map = None
...
def _compute_and_save(args):
    index, project_id, member_article_ids = args
    save_path = _save_dir / f'{index}_{project_id}_variables.csv'
    if save_path.exists():
        return

    project_dict = init_aggregated_article_project_dict(project_id)
    for article_id in member_article_ids:
        project_dict = compute_article_page_variables(
            article_id, project_dict, _bots_map, _bot_cat_voting_rule_lists
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Compute aggregated article page variables per WikiProject.'
    )
    ...
    save_dir = VARIABLES_DIR / 'article_page'
    ...
    work_items = [
        (index, row['project_page_id'], row['member_article_ids'])
        for index, row in project_to_articles_df.iterrows()
    ]

    if num_processes == 1:
        # Sequential (demo mode or single-core machine)
